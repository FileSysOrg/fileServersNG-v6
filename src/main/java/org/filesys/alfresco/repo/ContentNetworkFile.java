...
public class ContentNetworkFile extends NodeRefNetworkFile
{
    ...
    public static ContentNetworkFile createFile(NodeService nodeService, ContentService contentService, MimetypeService mimetypeService,
                                                SMBHelper cifsHelper, NodeRef nodeRef, String path, boolean readOnly, boolean attributesOnly, SrvSession sess)
    {
        ...
        if (attributesOnly) {
        	netFile.setGrantedAccess(NetworkFile.Access.ATTRIBUTES_ONLY);
        }
        else if (readOnly) {
        ...
        if ( netFile.isReadOnly() && netFile.getGrantedAccess() == NetworkFile.Access.READ_WRITE)
            netFile.setGrantedAccess(NetworkFile.Access.READ_ONLY);
    ...
    private boolean isWritable()
    {
        ...
        NetworkFile.Access access = getGrantedAccess();
        return (access == NetworkFile.Access.READ_WRITE || access == NetworkFile.Access.WRITE_ONLY);
    }
